...
def explore_city_data(city_data):
    ...
    print "Number of features:", np.size(housing_features, 1)
    ...
    print "Standard Deviation price", np.std(housing_prices)
...
def model_complexity_graph(max_depth, train_err, test_err):
    """Plot training and test error as a function of the depth of the decision
     tree learn."""
    print max_depth[np.argmin(test_err)]
    pl.figure()
...
def fit_predict_model(city_data):
    ...
    regressors = GridSearchCV(regressor, parameters, scoring=scorer,
                              verbose=True)
    regressors.fit(X, y)
